...
function Dropzone({ stage }: { stage: DocStage }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-[13px] text-[#475569] text-center">
        ...
        <button type="button" className="text-[#1B4E9B] underline-offset-2 hover:underline">
          выберите
        </button>
      </div>
...
export default function UploadScreen({ objectId, onBack, onRunCheck }: Props) {
  ...
  const totalSizeMb = 148; // демонстрационное значение из ТЗ
  const maxSizeMb   = 200;
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-[#F5F7FA]">
      ...
      {/* Шапка объекта: адрес, застройщик, разрешение */}
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-lg p-4">
              ...
              {registryLoaded ? (
                ...
                <Button
                  ...
                  onClick={() => setRegistryLoaded(true)}
                >
                  Загрузить реестр
                </Button>
              )}
